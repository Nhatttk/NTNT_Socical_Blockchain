...
import { Row, Form, Button, Card, Spinner, Col, Modal, InputGroup, Alert, Tabs, Tab } from 'react-bootstrap'
...
const Auctions = ({ contract }) => {
    ...
    const loadData = async () => {
        try {
            ...
            // Filter my posts
            const myPostsData = postsData.filter(post => 
            ...
            try {
                ...
                let auctionsWithDetails = await Promise.all(auctionsData.map(async auction => {
                    return await getAuctionWithDetails(auction, postsData, address)
                }))
                
                // Fetch unpaid auctions with details
                let unpaidAuctionsWithDetails = await Promise.all(unpaidAuctionsData.map(async auction => {
                    return await getAuctionWithDetails(auction, postsData, address)
                }))
                
                // Fetch my unpaid auctions with details
                let myUnpaidAuctionsWithDetails = await Promise.all(myUnpaidAuctionsData.map(async auction => {
                    return await getAuctionWithDetails(auction, postsData, address)
                }))
                
                // Remove null values (if any)
                auctionsWithDetails = auctionsWithDetails.filter(a => a !== null)
                unpaidAuctionsWithDetails = unpaidAuctionsWithDetails.filter(a => a !== null)
                myUnpaidAuctionsWithDetails = myUnpaidAuctionsWithDetails.filter(a => a !== null)
                ...
                setError("Hợp đồng thông minh chưa được cập nhật với chức năng đấu giá. Cần triển khai lại hợp đồng.")
    ...
    const getAuctionWithDetails = async (auction, postsData, userAddress) => {
        const post = postsData.find(p => p.id.toString() === auction.postId.toString())
        
        if (!post) return null
        
        // Fetch post metadata from IPFS
        let response = await fetch(`https://gateway.pinata.cloud/ipfs/${post.hash}`)
        ...
        return {
            ...
            isHighestBidder: auction.highestBidder.toLowerCase() === userAddress.toLowerCase(),
    ...
    const handleCreateAuction = async () => {
        try {
            ...
            const tx = await contract.createAuction(postIdBN, startPriceWei, durationBN)
            await tx.wait()
    ...
    const handleCancelAuctionPayment = async (auctionId) => {
        try {
            if (!window.confirm("Bạn có chắc muốn hủy đấu giá này vì chưa nhận được thanh toán?")) return
    ...
    const countdownRenderer = ({ days, hours, minutes, seconds, completed, auction }) => {
        if (completed) {
        ...
        <div className="container-fluid mt-5">
            ...
            {!isContractUpdated && (
                <Alert variant="info">
                    ...
                    <p>
                        Smart contract cần được cập nhật và triển khai lại để sử dụng tính năng đấu giá.
            ...
            {!hasProfile ? (
                ...
                <>
                    ...
                    >
                                            ...
                                            <Card>
                                                ...
                                                <Card.Body>
                                                    ...
                                                    <div className="mt-3">
                                                        <div className="d-flex justify-content-between">
                                                            ...
                                                            <strong>{ethers.utils.formatEther(auction.startPrice)} ETH</strong>
                                                        </div>
                                                            ...
                                                            <div className="d-flex align-items-center mt-2">
                                                                ...
                                                                <div className="ms-auto d-flex align-items-center">
                                                                    <img
                                                                        ...
                                                                        alt="Bidder Avatar"
                                                                    />
                                                                    <span>{auction.highestBidderProfile.username}</span>
                                                                </div>
                                                            </div>
                                                        )}
                                                        ...
                                                        <div className="d-flex justify-content-between mt-2">
                                                            ...
                                                            <strong className={auction.remainingTime < 300000 ? "text-danger" : ""}>
                                                                <Countdown 
                                                                    date={Date.now() + auction.remainingTime}
                                                                    renderer={(props) => countdownRenderer({...props, auction})}
                                                                />
                                                            </strong>
                                                        </div>
                                                    </div>
                                                ...
                                                <Card.Footer>
                                                    {auction.isOwner ? (
                                                        ...
                                                        >
                                                            {auction.isHighestBidder ? 'Bạn đang là người đặt giá cao nhất' : 'Đặt giá'}
                                                        </Button>
                                                    )}
                                                </Card.Footer>
                                            </Card>
                        ...
                        <Tab eventKey="unpaid" title={`Đấu giá cần thanh toán (${unpaidAuctions.length})`}>
                                                ...
                                                <Card.Body>
                                                    ...
                                                    <div className="mt-3">
                                                        ...
                                                        <Alert variant="warning" className="mt-3">
                                                            <Alert.Heading className="h6">Thanh toán ngay</Alert.Heading>
                                                            <p className="mb-0">
                                                                Bạn đã thắng đấu giá này. Vui lòng thanh toán để nhận bài viết.
                                                            </p>
                                                        </Alert>
                                                    </div>
                                                </Card.Body>
                        ...
                        <Tab eventKey="myunpaid" title={`Chờ thanh toán (${myUnpaidAuctions.length})`}>
                                            ...
                                            <Card border="info">
                                                ...
                                                <Card.Body>
                                                    ...
                                                    <div className="mt-3">
                                                        ...
                                                        <Alert variant="info" className="mt-3">
                                                            <Alert.Heading className="h6">Đang chờ thanh toán</Alert.Heading>
                                                            <p className="mb-0">
                                                                Người thắng đấu giá chưa thanh toán. Bạn có thể hủy đấu giá sau 1 giờ.
                                                            </p>
                                                        </Alert>
                                                    </div>
                                                </Card.Body>
                                                ...
                                                <Card.Footer>
                                                    <Button 
                                                        variant="danger" 
                                                        className="w-100"
                                                        onClick={() => handleCancelAuctionPayment(auction.id)}
                                                    >
                                                        Hủy đấu giá
                                                    </Button>
                                                </Card.Footer>
                    ...
                    <Form>
                        <Form.Group className="mb-3">
                            ...
                            <Form.Select 
                                ...
                                onChange={(e) => setCreateAuctionData({
                                    ...createAuctionData,
                ...
                <Modal.Body>
                    {currentAuction && (
                        <>
                            ...
                            <Alert variant="info">
                                <strong>Lưu ý:</strong> Khi đặt giá cao hơn, bạn đang cam kết mức giá này. Nếu bạn thắng đấu giá, bạn sẽ cần thanh toán số tiền này để nhận được bài viết.
                            </Alert>
                ...
                <Modal.Body>
                    {currentAuction && (
                        <>
                            ...
                            <p>
                                <strong>Số tiền cần thanh toán:</strong> {ethers.utils.formatEther(currentAuction.highestBid)} ETH
                            </p>
                            
                            <Alert variant="warning">
                                <strong>Lưu ý:</strong> Khi bạn thanh toán, số tiền sẽ được chuyển ngay cho chủ sở hữu bài viết và bài viết sẽ thuộc về bạn.
                            </Alert>
                        </>
                    )}
                </Modal.Body>
