...
const Home = ({ contract }) => {
  ...
  const deleteComment = async () => {
    ...
    try {
      console.log(
        `Deleting comment - Post ID: ${currentPostIdForComment}, Comment ID: ${commentToDelete.id}`
      );

      // Delete comment on blockchain - with explicit error handling
      const tx = await contract.deleteComment(
        currentPostIdForComment,
        commentToDelete.id
      );
                      ...
                      <span className="me-2">
                        {post.hasReacted ? (
                          <FaHeart
                            ...
                            onClick={() =>
                              hasProfile && toggleReaction(post.id)
                            }
                          />
                        ) : (
                          <FaRegHeart
